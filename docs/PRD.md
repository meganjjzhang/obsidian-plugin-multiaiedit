...
> 版本：v0.4 | 日期：2026-06-11 | 作者：婧晶 + 檐灯
...
- 复制 Prompt 包含完整原文 + 批阅意见。
- 导出批阅文件默认只保存批阅意见和上下文快照。
...
做法:**双 hash 模型**(修复了"自动更新 hash 永远检测不到变更"的逻辑漏洞):
...
| 产品 | 高亮 | 笔记 | 不污染原文 | 导出给 AI | CLI 一键执行 | Diff 确认 |
|------|------|------|------------|-----------|--------------|----------|
| Sidebar Highlights | 支持 | 支持 | 不支持 | 不支持 | 不支持 | 不支持 |
| HiNote | 支持 | 支持 | 不支持 | 部分支持 | 不支持 | 不支持 |
| Axl Light | 支持 | 基础 | 支持 | 不支持 | 不支持 | 不支持 |
| MultiAIEdit v0.1 | 支持 | 支持 | 支持 | 支持 | 不支持 | 不支持 |
| MultiAIEdit v0.2 | 支持 | 支持 | 支持 | 支持 | 支持 | 支持 |
